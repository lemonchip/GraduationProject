...
import { ReqGoodList,ReqGoodInfo,ReqGoodInfoColour,ReqGoodInfoSpecs } from '../api/product'
import { ReqHomeAdvertise,ReqHomeFlash } from '../api/home'
import { ReqAddCart,ReqloadCartByUser } from '../api/cart'
import { ReqcreateOrder,ReqcreateOrderItem,ReqGetMyOrder } from '../api/order'

// 获取秒杀数据
export const loadSeckillsInfoReq = ({ commit }) => {
  return new Promise((resolve, reject) => {

          ReqHomeFlash().then(res => {
            const data = res.data 
            if(data.code === 20000){
            ...
            commit('SET_SECKILLS_INFO', [result, deadline]);
    ...
    ReqHomeAdvertise().then(res => {
      ...
      if(result.code === 20000){
        const datas = {
          ...
          activity: [
            {
              img:'static/img/nav/nav_showimg1.jpg',
              category:'手机数码',
              size:15
            },
            {
              img:'static/img/nav/nav_showimg2.jpg',
              category:'手机数码',
              size:15
            }
          ]
        };
        commit('SET_CAROUSELITEMS_INFO', datas);
      }
    })
...
// 请求获得商品详细信息
export const loadGoodsInfoById = ({ commit },data) => {
  commit('SET_LOAD_STATUS', true);
  return new Promise((resolve, reject) => {
      console.log("data.param:"+data)
      ReqGoodInfo(data).then(res => {
          ...
          if(data.code === 20000){
            ...
            commit('SET_GOODS_INFO_ID', result);
            commit('SET_LOAD_STATUS', false);
          }
      });

      ReqGoodInfoColour(data).then(res => {
        ...
        if(data.code === 20000){
          ...
          const result = data.data
          console.log("result:"+JSON.stringify(result))
          commit('SET_GOODS_COLOUR', result);
          commit('SET_LOAD_STATUS', false);
        }
    });

    ReqGoodInfoSpecs(data).then(res => {
...
export const loadGoodsListByCategory = ({ commit },state) => {
  commit('SET_LOAD_STATUS', true);
  return new Promise((resolve, reject) => {

    ReqGoodList(state).then(response => {
              // let responseS = JSON.stringify(response)
              // let dataS = JSON.stringify(response.data)
              ...
              if(data.code === 20000){
                ...
                const datat = {
                  asItems: [],
                  goodsList:Gooddata.records  
              }

              commit('SET_GOODS_LIST', datat);
              commit('SET_LOAD_STATUS', false);
              resolve(true)
            }else{
              commit('SET_GOODS_LIST', {asItems: [], goodsList:[]});
            }
...
export const loadShoppingCart = ({ commit }) => {
  return new Promise((resolve, reject) => {
    if(localStorage.getItem('loginInfo') == undefined || localStorage.getItem('loginInfo') == null){
          const massage = '你未登陆，请登录'
          resolve(massage)
    }
    ...
    ReqloadCartByUser(data).then(res => {

      const resData = res.data 
      if(resData.code === 20000){
        console.log("data.data.records:"+resData.data.records)
        commit('SET_SHOPPING_CART', resData.data.records);
...
export const login = ({ commit }, data) => {
  return new Promise((resolve, reject) => {
    ...
    Reqlogin(data.username,data.password).then( response => {
        const data = response.data
       let responseS = JSON.stringify(response)
       let dataS = JSON.stringify(data)
        ...
        if (data.code === 20000) {
          ...
          console.log("data.data.user.closername:"+data.data.user.closername)
          console.log("data:"+data.data)
          commit('SET_USER_LOGIN_INFO', data.data);
          resolve(true);
          return true;
        }else {
          resolve(false);
...
//创建订单
export const createOrder = ({ commit }, data) => {
  return new Promise((resolve, reject) => {

    ReqcreateOrder(data.orderData).then( response => {
      const result = response.data
       ...
       let dataS = JSON.stringify(result)
        ...
        if (result.code === 20000) {
          ...
          item.orderSn = result.data.orderSn
          ReqcreateOrderItem(item).then( res => {
            const result = res.data
            if(result.code === 20000){
  ...
  export const loadMyOrder = ({ commit }, data) => {
    return new Promise((resolve, reject) => {
  
      ReqGetMyOrder(data).then( response => {
          const data = response.data
         //let responseS = JSON.stringify(response)
         let dataS = JSON.stringify(data)
          console.log("data:"+dataS)
          if (data.code === 20000) {
            ...
            commit('SET_USER_ORDER_LIST', data.data.records);
            resolve(true);
            return true;
          }else {
            resolve(false);
