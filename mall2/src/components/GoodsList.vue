        ...
        <div class="as-box">
          ...
          <div class="item-as" v-for="(item,index) in asItems" :key="index">
            <div class="item-as-img">
              <img :src="item.img" alt="" style="width: 160px;height: 160px;">
            </div>
          ...
          <div class="goods-list">
            <div class="goods-show-info" v-for="(item, index) in orderGoodsList" :key="index">
              <div class="goods-show-img">
                <router-link :to="{path:'/goodsDetail',query:{productID:item.id}}" >
                <img :src="item.pic" style="width: 160px;height: 160px;" />
                </router-link>
              </div>
              <div class="goods-show-price">
                <span>
                  <Icon type="social-yen text-danger"></Icon>
                  <span class="seckill-price text-danger">{{item.originalPrice}}</span>
                </span>
              </div>
...
export default {
  ...
  methods: {
    ...mapActions(['loadGoodsListByCategory']),
  ...
  created () {
    ...
    this.category = this.$route.query.category;//获取上个页面传递的category,在下面获取数据的时候先提交category
    const state = {
      category:this.category,
      page:this.page
    }
   // this.loadGoodsList()
    this.loadGoodsListByCategory(state);
